...
class TeacherService{
  static Future <dynamic> getTeacherList(String token, int page) async{
    var dio = Dio();
    try{
  ...
  static Future <List<Tutor>> searchTeacher(String token, int page, int perPage, {String name = "", List<String> nationality = const [], String date = "", String time = "", List<String> specialities = const []}) async{
    var dio = Dio();
    List<Tutor> list = [];
    List<String> spec = [];
  ...
  static Future <dynamic> getTeacherDetail(String token, String id) async{
    var dio = Dio();
    try{
      // dio.options.headers["Authorization"] = "Bearer ${context.watch<Tokens>().access.token}";
      dio.options.headers["Authorization"] = "Bearer ${token}";
  ...
  static Future<dynamic> getTutorSchedule(String token, String tutorId) async {
    var dio = Dio();
    try{
      // dio.options.headers["Authorization"] = "Bearer ${context.watch<Tokens>().access.token}";
  ...
  static Future <dynamic> addFavoriteTeacher(String token, String id) async{
    var dio = Dio();
    try{
      // dio.options.headers["Authorization"] = "Bearer ${context.watch<Tokens>().access.token}";
      dio.options.headers["Authorization"] = "Bearer $token";
  ...
  static Future <bool> reportTeacher(String token, String id, String reason) async{
    var dio = Dio();
    try{
      dio.options.headers["Authorization"] = "Bearer $token";
